...
- To declare localization strings, specify them in the `resources_en_US.json` file in `src/assets/locales`. Using JSON structure, specify a string name and provide the string and a description. Then you can reference the localization strings with the `skyAppResources` pipe or the `SkyAppResourcesService` service in `@blackbaud/skyux-builder/runtime/i18n`.
